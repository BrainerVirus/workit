...
import { createTemplateTools } from "../src/tools/templates";
...
test("template_list reports repo sources by default; template_edit writes config", async () => {
  ...
  try {
    ...
    expect(list.data.templates.some((t: any) => t.name === "issue-update" && t.source === "repo")).toBe(true);

    const edit = JSON.parse(await tools.workflow_template_edit.execute({
      name: "issue-update", content: "# Custom\n", confirmed: true,
    }, {} as never) as string);
    expect(edit.ok).toBe(true);
    expect(edit.data.path).toContain(dir);

    const no = JSON.parse(await tools.workflow_template_edit.execute({
      name: "issue-update", content: "# X\n", confirmed: false,
    }, {} as never) as string);
