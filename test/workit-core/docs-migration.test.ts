...
test(
  "MIGRATION_CHOICES are exactly Migrate safely and Not now",
  () => {
...
test(
  "detect pairs through explicit plan links first",
  () => {
    const root = tmp();
    try {
      putLegacy(root, "foo", {
...
test(
  "detect pairs through filename fallback when no explicit link",
  () => {
    const root = tmp();
    try {
      putLegacy(root, "foo");
...
test(
  "detect scans only docs/superpowers and flags top-level files as ambiguous",
  () => {
    const root = tmp();
    try {
      putLegacy(root, "foo");
...
test(
  "detect reports orphaned spec-only, plan-only, and sdd-only entries",
  () => {
    const root = tmp();
    try {
...
test(
  "detect marks colliding target slugs ambiguous and unsafe",
  () => {
    const root = tmp();
    try {
...
test(
  "migrate without confirmation declines, writes nothing, and flags the active workflow",
  () => {
    const root = tmp();
    try {
...
test(
  "migrate decline for an unrelated slug is not an active-workflow block",
  () => {
    const root = tmp();
    try {
      putLegacy(root, "foo", { sdd: true });
...
test(
  "confirmed migrate rescans legacy state added after an earlier call",
  () => {
    const root = tmp();
    try {
...
test(
  "migrate aborts atomically on differing destinations with no partial copy",
  () => {
    const root = tmp();
    try {
      putLegacy(root, "foo");
...
test(
  "migrate treats identical destinations as already migrated and is idempotent",
  () => {
    const root = tmp();
    try {
      putLegacy(root, "foo");
...
test(
  "migrate treats a destination symlink to a directory as a collision, not a throw (D4)",
  () => {
    const root = tmp();
    try {
      putLegacy(root, "foo");
...
test(
  "migrate copies a paired workflow and preserves sources byte-identical",
  () => {
    const root = tmp();
    try {
      putLegacy(root, "foo");
...
test(
  "migrate copies symlinked file content and refuses symlink escapes atomically",
  () => {
    const root = tmp();
    try {
      gitRepo(root);
...
test(
  "migrate copies a symlinked source as regular content bytes",
  () => {
    const root = tmp();
    try {
      gitRepo(root);
...
test(
  "migrate does not false-refuse a workspace reached through a symlinked path segment",
  () => {
...
test(
  "migrate retries safely after a partial copy failure",
  () => {
    const root = tmp();
    try {
      gitRepo(root);
      ...
      expect(readFileSync(path.join(root, "docs", "foo", "spec.md"), "utf8")).toBe(
        legacySpec("foo"),
      );
...
test(
  "migrate rewrites copied plan links and flow paths to canonical locations only",
  () => {
    const root = tmp();
    try {
      gitRepo(root);
...
test(
  "migrate preserves malformed plan and flow state byte-identical and reports it",
  () => {
    const root = tmp();
    try {
      gitRepo(root);
...
test(
  "migrate refuses SDD copy until the canonical SDD ignore contract is active",
  () => {
    const root = tmp();
    try {
      putLegacy(root, "foo", { sdd: true, flow: true });
...
test(
  "migrate copies SDD state when docs/*/sdd is gitignored",
  () => {
    const root = tmp();
    try {
      gitRepo(root);
...
test(
  "migrate copies both workflows sharing a symlinked sdd dir (per-entry visits)",
  () => {
    const root = tmp();
    try {
      gitRepo(root);
...
test(
  "migrate refuses a docs/<slug> symlink escaping the workspace and writes nothing outside",
  () => {
    const root = tmp();
    try {
      putLegacy(root, "foo");
...
test(
  "migrate does not rewrite sibling-name-prefix legacy references",
  () => {
    const root = tmp();
    try {
      gitRepo(root);
...
test(
  "migrate aborts cleanly on a dangling symlink instead of throwing",
  () => {
    const root = tmp();
    try {
      gitRepo(root);
...
test(
  "migrate cannot create a divergent active workflow when SDD copy is refused",
  () => {
    const root = tmp();
    try {
      putLegacy(root, "foo", { sdd: true, flow: true });
