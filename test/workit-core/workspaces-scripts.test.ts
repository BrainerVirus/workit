...
test(
  "config.sh resolve: workspace match wins over global vcs.json (work -> gitlab, personal -> github)",
  () => {
...
test(
  "config.sh resolve: WORKFLOW_TOOLKIT_CONFIG dir is honored for vcs.json + workspaces.json (TS chain parity)",
  () => {
    const base = realpathSync(mkdtempSync(path.join(os.tmpdir(), "wf-ws-dir-")));
...
test(
  "config.sh resolve: WORKFLOW_WORKSPACE_ROOT overrides the cwd",
  () => {
    const base = realpathSync(mkdtempSync(path.join(os.tmpdir(), "wf-ws-dir-")));
    mkdirSync(path.join(base, "work", "repo"), { recursive: true });
...
test(
  "config.sh resolve: catchall ** matches a drive-letter WORKFLOW_WORKSPACE_ROOT (TS parity)",
  () => {
    const r = withConfigFiles(
      {
        "vcs.json": JSON.stringify(GLOBAL_VCS),
...
test(
  "config.sh load: merges the resolved workspace over global vcs.json",
  () => {
    const base = realpathSync(mkdtempSync(path.join(os.tmpdir(), "wf-ws-dir-")));
    mkdirSync(path.join(base, "work", "repo"), { recursive: true });
    ...
    try {
      const m = withConfigFiles(files, {}, () =>
        vcsConfig("load", path.join(base, "work", "repo")),
      );
...
test(
  "pr-create.sh --build-body: branch-derived issue id appends Related to line",
  () => {
    expect(
      buildBody({
...
test(
  "pr-create.sh --build-body: link_issues false or absent -> body unchanged",
  () => {
    expect(
      buildBody({
        BODY: "Same body",
...
test(
  "pr-create.sh --build-body: version-like tokens are not linked (POSTGRES-16, HTTP-3), issue refs are",
  () => {
    expect(
      buildBody({
...
test(
  "pr-create.sh --build-body: date-like branch prefixes never close an unrelated issue (RL-03/CA-25)",
  () => {
...
test(
  "pr-create.sh --build-body: explicit WORKFLOW_YT_ISSUE wins over branch-derived id",
  () => {
    expect(
      buildBody({
        BRANCH: "feature/IRP-123-fix",
...
test(
  "pr-create.sh --build-body: no base URL or no derivable id -> no Related to line",
  () => {
...
test(
  "pr-create.sh --build-body: explicit WORKFLOW_GH_ISSUE closes/related (default closes, # prefix stripped)",
  () => {
...
test(
  "pr-create.sh --build-body: branch-derived pure-number id auto-links (feature/42-title -> Closes #42)",
  () => {
    expect(buildBody({ GH_LINK_ON_PR: "true", BRANCH: "feature/42-title" })).toBe("Closes #42");
...
test(
  "pr-create.sh --build-body: owner/repo parsed from git remote get-url origin; absent remote -> Related line without URL",
  () => {
...
test(
  "pr-create.sh create: cfg-driven wiring emits Closes #N into the real gh invocation",
  () => {
    ...
    try {
      ...
      expect(
        spawnSync("git", ["add", "README.md"], { cwd: repoDir, encoding: "utf8" }).status,
      ).toBe(0);
      expect(
