...
test(
  "CA-20: coordinator product edits are blocked after subagent-driven selection",
  () => {
...
test(
  "CA-20: coordinator stays allowed when the menu was not subagent-driven",
  () => {
    const { root, slug } = fixture();
    try {
      writeDocs(root, slug);
...
test(
  "CA-20: authenticated delegated workers are allowed; unauthenticated ones are not",
  () => {
    const { root, slug } = fixture();
    try {
      establishSubagentDriven(root, slug);
...
test(
  "CA-21: active discovery uses the host workspace, not process.cwd()",
  () => {
    const { root, slug } = fixture();
...
test(
  "FG-08/CA-21: unique per-write temp names never reuse a shared <file>.tmp",
  () => {
    const { root, slug } = fixture();
    try {
      writeDocs(root, slug);
...
test(
  "FG-08: stale concurrent writers are preserved via compare/retry, never clobbered",
  () => {
    const { root, slug } = fixture();
    try {
      writeDocs(root, slug);
...
test(
  "FG-08: a transition re-reads and retries on the fresh state after a concurrent commit",
  () => {
    const { root, slug } = fixture();
    try {
      writeDocs(root, slug);
...
test(
  "CA-28: a failed-session fixture cannot be skipped and keeps flow state isolated",
  () => {
    const { root, slug } = fixture();
    try {
...
test(
  "FG-08: two writers with the same expected text — only one wins",
  () => {
    const { root, slug } = fixture();
    try {
      writeDocs(root, slug);
...
test(
  "A4: an EACCES flow write surfaces io_error (never conflict) and leaves no .tmp behind",
  () => {
...
test(
  "A4: a transition with an unwritable flow store returns flow_io_error, not a conflict retry",
  () => {
    const { root, slug } = fixture();
    const sddDir = path.join(root, "docs", slug, "sdd");
    try {
...
test(
  "A5: a mutation context for another workspace is rejected with workspace_mismatch",
  () => {
    const { root, slug } = fixture();
...
test(
  "CA-20/AR-12: delegation derives from host parentage, and delegated contexts pass the product gate",
  () => {
    const { root, slug } = fixture();
    try {
      establishSubagentDriven(root, slug);
...
test(
  "CA-18/AR-13: root-session interception blocks write tools and mutating shell once subagent-driven is active",
  () => {
    const { root, slug } = fixture();
    try {
      establishSubagentDriven(root, slug);
...
test(
  "CA-19: a held flow.json.lock yields flow_concurrent_conflict, then recovers",
  () => {
    const { root, slug } = fixture();
    try {
      writeDocs(root, slug);
      ...
      const recovered = transitionSpec(
        root,
        slug,
        `docs/${slug}/spec.md`,
        evidence("Approve spec"),
...
test(
  "CA-19: a stale flow.json.lock (older than STALE_LOCK_MS) is recovered instead of a permanent conflict",
  () => {
    const { root, slug } = fixture();
    try {
      writeDocs(root, slug);
...
test(
  "CA-19: a leftover .lock.stale from a crashed recovery is removed on the next acquisition",
  () => {
    const { root, slug } = fixture();
    try {
      writeDocs(root, slug);
...
test(
  "CA-19: activation is a locked critical section: prepareFlowState under a held lock conflicts",
  () => {
    const { root, slug } = fixture();
    try {
      writeDocs(root, slug);
...
test(
  "CA-19: a fresh flow.json.lock is NOT reclaimed as stale and still yields flow_concurrent_conflict",
  () => {
    const { root, slug } = fixture();
    try {
      writeDocs(root, slug);
...
test(
  "CA-19: an unremovable stale lock returns flow_concurrent_conflict and never runs the critical section",
  () => {
    const { root, slug } = fixture();
    try {
      writeDocs(root, slug);
...
test(
  "CA-19: releasing a lock never deletes a successor's lock at the same path",
  () => {
    const { root, slug } = fixture();
...
test(
  "A4: a read-path reconcile persist failure returns flow_io_error, never throws through the lock",
  () => {
    const { root, slug } = fixture();
    try {
      writeDocs(root, slug);
...
test(
  "CA-19: reconciliation and a reapproval serialize into one winning state with the new digest",
  () => {
    const { root, slug } = fixture();
    try {
      writeDocs(root, slug);
      ...
      const reapproved = transitionPlan(
        ...
        slug,
        plan,
        openEvidence(store, "s1", "Approve plan"),
      );
...
test(
  "CA-19: a reconcile reset persists atomically and a later reapproval wins",
  () => {
    const { root, slug } = fixture();
    try {
      writeDocs(root, slug);
...
test(
  "CA-18/CA-19: malformed flow.json stays byte-identical through effective reads",
  () => {
    const { root, slug } = fixture();
    try {
      writeDocs(root, slug);
...
test(
  "CA-19/CA-23: completion runs the verifier outside the lock and rejects a concurrent state change",
  () => {
    const { root, slug } = fixture();
    try {
      establishSubagentDriven(root, slug);
...
test(
  "CA-18/AR-13: only active subagent-driven flows are discovered and intercepted",
  () => {
...
test(
  "fail-closed scan: a locked flow is reported as a read error, not silently inactive",
  () => {
    const { root, slug } = fixture();
    try {
      establishSubagentDriven(root, slug);
...
test(
  "CA-12: subagent-driven activation persists the activating coordinator session id",
  () => {
    const { root, slug } = fixture();
    try {
      establishSubagentDriven(root, slug);
...
test(
  "CA-12..CA-17: literal clean-start contract — menu record -> branch setup -> coordinator brief -> direct worker -> coordinator review/progress with no error result",
  () => {
    const { root, slug } = fixture();
    const plan = `docs/${slug}/plan.md`;
    try {
      ...
      const workerControl = assertSddControlGates(
        root,
        slug,
        { requireMenu: true },
        delegated(root),
...
test(
  "CA-16: composed opencode-path clean start — receipts to review package with zero failed calls",
  () => {
    const root = mkdtempSync(path.join(os.tmpdir(), "wf-concurrency-"));
    const slug = "conc-flow";
    const plan = `docs/${slug}/plan.md`;
