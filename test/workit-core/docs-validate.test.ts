...
test(
  "workit_docs_validate accepts a contiguous linked pair",
  async () => {
...
test(
  "workit_docs_validate hard-fails on task number gap",
  async () => {
    const { root, spec, plan } = fixture();
    writeFileSync(
      path.join(root, plan),
...
test(
  "workit_docs_validate hard-fails on Spec link or branch mismatch",
  async () => {
    const { root, spec, plan } = fixture();
    writeFileSync(
      path.join(root, plan),
...
test(
  "sdd_not_ignored when sdd dir exists and is not gitignored",
  async () => {
...
test(
  "no sdd_not_ignored when sdd is gitignored",
  async () => {
...
test(
  "docsValidate rejects cross-slug pairs through the shared resolver",
  () => {
    const { root } = fixture();
    try {
...
test(
  "docsValidate rejects wrong basenames through the shared resolver",
  () => {
...
test(
  "docsValidate rejects traversal through the shared resolver",
  () => {
...
test(
  "docsValidate rejects absolute paths through the shared resolver",
  () => {
    const { root, spec, plan } = fixture();
    try {
      const result = docsValidate({
...
test(
  "docsValidate reports hygiene warnings",
  async () => {
