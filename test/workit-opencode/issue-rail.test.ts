...
test("CA-01: accepts both input shapes — array and { questions }", () => {
  ...
  expect(
    detectInstructionOption({ questions: [{ options: [{ label: "Enter the issue number" }] }] }),
  ).toBe(true);
});
...
test("CA-02: question part input feeds the detector (state.input shape)", () => {
  const input = (
    questionPart({
      questions: [{ header: "q", options: [{ label: "Type the issue URL/ID" }] }],
    }) as { state?: { input?: unknown } }
  ).state?.input;
  expect(detectInstructionOption(input)).toBe(true);
  expect(
    detectInstructionOption(
      (
        questionPart([{ options: [{ label: "Enter the issue number" }] }]) as {
          state?: { input?: unknown };
        }
      ).state?.input,
    ),
  ...
  expect(
    detectInstructionOption(
      (
        questionPart({ questions: [{ options: [{ label: "Skip" }] }] }) as {
          state?: { input?: unknown };
        }
      ).state?.input,
    ),
  ...
  expect(
    detectInstructionOption((questionPart(null) as { state?: { input?: unknown } }).state?.input),
  ).toBe(false);
});
...
test("CA-02: hook injects the rail when the previous assistant used an instruction-labeled question option", async () => {
  ...
  const output = {
    messages: [
      userMessage("start"),
      assistantMessageWithQuestion({
        questions: [{ header: "h", options: [{ label: "Type the issue URL/ID" }] }],
      }),
...
test("CA-02: no injection without an instruction-labeled option, or when the marker is already present", async () => {
  ...
  const clean = {
    messages: [
      assistantMessageWithQuestion({
        questions: [{ header: "h", options: [{ label: "Skip" }, { label: "Use session facts" }] }],
      }),
      userMessage("continue"),
    ],
  };
  await hooks["experimental.chat.messages.transform"]?.({} as never, clean as never);
  const cleanText = clean.messages[1].parts
    .filter((p: any) => p.type === "text")
    .map((p: any) => p.text)
    .join("\n");
  ...
  const twice = {
    messages: [
      userMessage("start"),
      assistantMessageWithQuestion({
        questions: [{ header: "h", options: [{ label: "Paste the URL" }] }],
      }),
      userMessage("continue"),
    ],
  };
...
test("CA-02: 'ir' tag is present and unique across makePart tags in plugin.ts", () => {
  const src = readFileSync(
    path.join(root, "packages", "workit-opencode", "src", "plugin.ts"),
    "utf8",
  );
  const tags = [...src.matchAll(/makePart\([^,]+(?:,\s*"([^"]+)")?\)/g)].map((m) => m[1] ?? "r");
