...
test(
  "approval and menu tool schemas expose no evidence, role, or taskIdentity fields",
  async () => {
    const { tools } = fixture();
...
test(
  "a caller-supplied evidence object is inert: no receipt means approval fails",
  async () => {
...
test(
  "a host-issued question receipt is consumed by the approval tool without evidence args",
  async () => {
...
test(
  "an intervening unrelated question cannot mask menu evidence (CA-02 purpose binding)",
  async () => {
    const { root, tools, ctx, receipts } = fixture();
    try {
      await run(tools, "workit_flow_status", { plan_path: "docs/oc-flow/plan.md" }, ctx);
...
test(
  "a negative answer cannot be laundered into an approval (FINDING 3)",
  async () => {
...
test(
  "a negative answer cannot be laundered into a menu choice (FINDING 3)",
  async () => {
...
test(
  "receipt replay fails: one receipt approves exactly once",
  async () => {
    const { root, tools, ctx, receipts } = fixture();
    try {
      await run(tools, "workit_flow_status", { plan_path: "docs/oc-flow/plan.md" }, ctx);
...
test(
  "FINDING 5: two concurrent approve calls with ONE receipt — exactly one succeeds",
  async () => {
    const { root, tools, ctx, receipts } = fixture();
    try {
      await run(tools, "workit_flow_status", { plan_path: "docs/oc-flow/plan.md" }, ctx);
...
test(
  "a receipt recorded for another session cannot be consumed",
  async () => {
    const { root, tools, ctx, receipts } = fixture();
    try {
      await run(tools, "workit_flow_status", { plan_path: "docs/oc-flow/plan.md" }, ctx);
...
test(
  "menu consumption binds the exact selected label: a mismatched receipt label fails",
  async () => {
    const { root, tools, ctx, receipts } = fixture();
    try {
      await run(tools, "workit_flow_status", { plan_path: "docs/oc-flow/plan.md" }, ctx);
...
test(
  "menu consumption accepts the capitalized label exactly as the user answers it",
  async () => {
    const { root, slug, tools, ctx, receipts } = fixture();
    try {
      await run(tools, "workit_flow_status", { plan_path: "docs/oc-flow/plan.md" }, ctx);
...
test(
  "menu consumption accepts host-decorated receipt labels for every source choice",
  async () => {
...
test(
  "a real child session (host parentage) is delegated and denied control metadata",
  async () => {
...
test(
  "the root session owns control metadata after subagent-driven; caller role args are inert",
  async () => {
...
test(
  "a failing session lookup fails closed to the coordinator: control writes stay root-owned",
  async () => {
...
test(
  "full flow through the opencode tools with host receipts: approvals + menu + gated SDD writes",
  async () => {
    const { root, tools, ctx, receipts } = fixture();
    try {
      await run(tools, "workit_flow_status", { plan_path: "docs/oc-flow/plan.md" }, ctx);
...
test(
  "workit_flow_status prepares activation and canonical paths on first read",
  async () => {
    const { root, tools, ctx } = fixture();
    try {
      const out = await run(
...
test(
  "lifecycle tool schemas expose only plan_path and no caller evidence/role fields",
  async () => {
    const { tools } = fixture();
...
test(
  "workit_flow_status returns execution and drift alongside spec/plan/menu",
  async () => {
    const { root, tools, ctx } = fixture();
    try {
      const out = await run(
...
test(
  "workit_flow_status reports digest drift while preserving the execution lifecycle after the plan changes",
  async () => {
    const { root, slug, tools, ctx, receipts } = fixture();
    try {
      await run(tools, "workit_flow_status", { plan_path: "docs/oc-flow/plan.md" }, ctx);
...
test(
  "lifecycle tools: active -> paused -> active -> completed with one-use receipts",
  async () => {
    const { root, slug, tools, ctx, receipts } = fixture();
    try {
      await run(tools, "workit_flow_status", { plan_path: "docs/oc-flow/plan.md" }, ctx);
...
test(
  "a wrong lifecycle purpose does not consume the receipt (purpose isolation: pause vs resume)",
  async () => {
    const { root, slug, tools, ctx, receipts } = fixture();
    try {
      await run(tools, "workit_flow_status", { plan_path: "docs/oc-flow/plan.md" }, ctx);
...
test(
  "a negative lifecycle answer cannot be laundered into a pause",
  async () => {
...
test(
  "a stale lifecycle receipt cannot pause a flow",
  async () => {
    const { root, slug, tools, ctx, receipts } = fixture();
    try {
      await run(tools, "workit_flow_status", { plan_path: "docs/oc-flow/plan.md" }, ctx);
...
test(
  "resume after plan drift still works — lifecycle survives plan-doc drift",
  async () => {
    const { root, slug, tools, ctx, receipts } = fixture();
    try {
      await run(tools, "workit_flow_status", { plan_path: "docs/oc-flow/plan.md" }, ctx);
...
test(
  "lifecycle tools preserve coordinator/delegated parentage and caller role args are inert",
  async () => {
