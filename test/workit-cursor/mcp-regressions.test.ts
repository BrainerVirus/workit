...
test(
  "changelog rejects missing entries without throwing",
  () => {
...
test(
  "changelog rejects a path outside workspace_root",
  () => {
...
test(
  "handoff includes every parsed task row",
  () => {
    const root = temporaryDirectory();
    try {
...
test(
  "docs validate rejects task number gaps",
  () => {
    const root = temporaryDirectory();
    try {
...
test(
  "question choices use Cursor AskQuestion without an MCP adapter",
  () => {
    const targets = [
...
test(
  "repository-scoped Cursor skill calls pass the active workspace_root",
  () => {
...
test(
  "wk-implement strips the handoff destination section so inline executors are not misclassified",
  () => {
...
test(
  "YouTrack post orchestration accepts injected operations",
  () => {
...
test(
  "YouTrack partial time failure never hides a posted comment",
  async () => {
    let comments = 0;
    let timeLogs = 0;
...
test(
  "release notes require and expose an explicit range",
  () => {
    const root = temporaryDirectory();
    try {
      spawnSync("git", ["init", "-q"], { cwd: root });
...
test(
  "plugin and MCP versions are synchronized",
  () => {
...
test(
  "build scripts derive their directory with fileURLToPath, not URL pathname",
  () => {
    for (const rel of [
...
test(
  "cursor MCP manifests stay package-relative (mcp.json, marketplace.json, hooks-cursor.json)",
  () => {
    ...
    const hooks = JSON.parse(
      readFileSync(path.join(CURSOR_ROOT, "hooks/hooks-cursor.json"), "utf8"),
    );
...
test(
  "root tsconfig includes Cursor MCP source in strict typechecking",
  () => {
...
test(
  "cursor MCP server registers the full required tool surface",
  () => {
...
test(
  "docs validate accepts the new docs/<slug> layout",
  () => {
    const root = temporaryDirectory();
    try {
...
test(
  "changelog accepts array entries with type/text",
  () => {
    const root = temporaryDirectory();
    try {
      writeFileSync(
...
test(
  "changelog rejects entries missing category or text",
  () => {
    expect(
...
test(
  "changelog rejects invalid categories",
  () => {
    expect(
...
test(
  "changelog rejects non-object non-array entries",
  () => {
...
test(
  "changelog stats: missing file, no unreleased, and duplicate headings",
  () => {
    const root = temporaryDirectory();
    try {
...
test(
  "present renders ascii and mermaid via real scripts",
  () => {
...
test(
  "present surfaces script failures",
  () => {
...
test(
  "no docs/superpowers paths remain in sources",
  () => {
...
test(
  "no wf- entry-point references remain in skills, commands, or plugin descriptions",
  () => {
    const { readdirSync, statSync } = require("node:fs") as typeof import("node:fs");
...
test(
  "no python3 invocations remain in packages/* (CA-11)",
  () => {
