...
test("Cursor build, package, and packed CLI doctor enforce exact canonical inert skills and Workit identity", () => {
  ...
  try {
    ...
    rmSync(path.join(missingRepo, "packages/workit-core/vendor/superpowers/skills/brainstorming"), {
      recursive: true,
    });
    ...
    try {
      expect(skillManifests(path.join(extracted.packageDir, "vendor/superpowers/skills"))).toEqual(
        SUPERPOWERS,
      );
    ...
    for (const damaged of [
      ...
      expect(missing.report.checks.find((check: any) => check.id === "assets").status).toBe("fail");
