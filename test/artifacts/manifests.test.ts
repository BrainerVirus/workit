...
test(
  "cursor mcp.json launches the published package via npx (CA-17)",
  () => {
...
test(
  "active Cursor runtime selectors use the canonical @latest + --prefer-online (except the doctor negative fixture)",
  () => {
...
test(
  "cursor .cursor-plugin/plugin.json uses valid plugin-root-relative components",
  () => {
    const packs = packWorkspacePackages();
...
test(
  "root .cursor-plugin/marketplace.json indexes packages/workit-cursor (CA-13)",
  () => {
    const market = json<{
      name: string;
      owner?: { name: string };
...
test(
  "package plugin manifest carries complete metadata and a committed logo (CA-14)",
  () => {
    const plugin = json<Record<string, unknown>>(
...
test(
  "obsolete flat package marketplace.json is absent (CA-13)",
  () => {
...
test(
  "opencode package.json ships a package-relative plugin entry and pins the SDK build-only",
  () => {
    const pkg = json<{
      main: string;
...
test(
  "all platform packages declare the Node minimum and publish no OpenCode SDK runtime dependency",
  () => {
    const packs = packWorkspacePackages();
...
test(
  "ci.yml pins the declared support matrix (Bun/Node/OpenCode, 3 OS, no Deno)",
  () => {
...
test(
  "bun.lock pins the declared Bun types and the current OpenCode SDK",
  () => {
