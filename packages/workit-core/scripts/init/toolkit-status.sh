...
if placeholder:
    ...
    status["next_step"] = (
        "Open the YouTrack create-token URL, New token (name workit, scope YouTrack), "
        "paste into the token file, save, then run /wk-status"
        if create_url
        else "Open the YouTrack token file, replace YOUR_TOKEN_HERE, save, then run /wk-status"
    )
elif vcs_json_ok and vcs_placeholder:
    path = vcs_token_item.get("token_edit_path") or ""
    status["vcs_token_edit_path"] = path
    status["next_step"] = (
        f"Open {path}, paste your {provider} token, save, then run /wk-status"
    )
elif not status["youtrack_ok"]:
    status["next_step"] = "Fix YouTrack token or re-run /wk-init"
elif vcs_json_ok and not status["vcs_ok"]:
    status["next_step"] = f"Fix {provider} token in vcs config or re-run /wk-init"
elif status["ready"]:
