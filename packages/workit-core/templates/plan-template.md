...
- The final task closes the lead Workit task with `workit_task` `action: "close"` (CLI: `workit task task close --payload … [--confirm]`) once requirements are satisfied and repository verification passes — never finish while the task is still `active` or `paused`.
