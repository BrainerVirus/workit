...
print(json.dumps({
    ...
    'ok':True,
    **d,
    'instruction':f'Open {p} in your editor, replace YOUR_TOKEN_HERE with your YouTrack permanent token, save, then run /wk-status',
}))
"
    ;;
  youtrack_scaffold)
...
print(json.dumps({
    ...
    "instruction": (
        f"Open the create-token URL, New token → name workit, scope YouTrack, "
        f"paste into {token_path}, then /wk-status."
    ),
}))
PY
    ;;
...
print(json.dumps({
    ...
    "instruction": (
        f"Open the create-token URL for {provider}, click Create, paste into {active_path}, then /wk-status."
    ),
}))
PY
    ;;
