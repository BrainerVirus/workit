---
name: wf-init
description: One-time workflow-toolkit setup — MCP deps, YouTrack, and VCS (GitLab/GitHub) config. Tokens edited locally only. Use /wf-init.
...
| Meeting time | `/wf-meetings` |
| Task updates | `/wf-issue-update` |
...
Both `gitlab.token` and `github.token` are always created (switch later by editing `provider` in `vcs.json`). Tell the user which token file is **active** for `/wf-pr` based on the chosen provider.
...
| Prefilled name | `token_name` → `workflow-toolkit` |
...
1. [Create YouTrack token](<token_create_url>) → **New token** → name **workflow-toolkit**, scope **YouTrack** → **Create token**
...
| Prefilled name | `workflow-toolkit` |
| GitLab scopes | `api` |
...
- Verification is **`/wf-status`** only — not part of init.
