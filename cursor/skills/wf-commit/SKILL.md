---
name: wf-commit
description: Structured local commits via workflow_git_context. Use for /wf-commit or "use commit skill" only — not casual "ship it".
...
Call MCP tool `workflow_git_context` with arguments from the user's message (paths after `/wf-commit`, etc.).
...
- `/wf-commit` → commit **all uncommitted changes**
- `/wf-commit <paths...>` → commit **only changes inside those paths**
