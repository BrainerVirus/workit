...
3. **Editable templates**: issue-update, greeting, and headers templates copied to `~/.config/workflow-toolkit/templates/`; new `workit_template_edit` tool edits them with agent assistance; YouTrack tools read templates from config with fallback to repo defaults.
4. **Canonical rules, multi-platform**: rules live in `~/.config/workflow-toolkit/rules/<name>/rule.md` (canonical frontmatter: `name`, `description`, `platforms`, markdown body). A compiler (`src/core/rules.ts`) translates them: → Cursor `.mdc` (description + alwaysApply) via sync-runtime; → OpenCode contract sections injected by the bootstrap. New `workit_rule_list` / `workit_rule_edit` tools. Repo defaults remain the base; user rules override.
...
`workit_init_apply` grows a guided mode: instead of env-only defaults, it asks (native questions, one at a time):
...
`workit_template_edit { name, confirmed }` — names: `issue-update|greeting|headers`. Reads the current template (config path, fallback repo), lets the agent apply user-driven edits, writes back to config, returns the diff. `workit_template_list` lists available templates and their source (config vs repo default).

YouTrack tools (`workit_youtrack_draft`, `context`) read `templates/issue-update.md` / `greeting.md` / `headers.md` from config when present, else repo templates. The `commentHeader`/`attachmentsHeader*` fields in `youtrack.json` become legacy (templates supersede them; keep reading them as fallback for compat).
...
Tools:
- `workit_rule_list`: lists merged rules (name, platform targets, source).
- `workit_rule_edit { name, confirmed }`: opens the canonical file (config copy; creates from repo default on first edit), applies agent-assisted edits, writes back; compiler artifacts regenerate on next sync/bootstrap.
...
- CA-03: `workit_template_edit`/`workit_template_list` read/write templates in the config dir with fallback to repo defaults.
- CA-04: Canonical rules compile to Cursor `.mdc` and OpenCode contract sections; user rules override repo defaults by name; `workit_rule_list`/`workit_rule_edit` expose them.
...
- Rule with unknown platform: compiler skips it and reports it in `workit_rule_list`.
...
- Template tests: `workit_template_edit` writes to config, tools read config with repo fallback.
