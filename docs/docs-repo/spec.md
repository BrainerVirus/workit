...
1. `workflow_docs_repo_link { path, confirmed }` — store the docs repo path in `~/.config/workflow-toolkit/docs-repo.json`; validate the path exists, is a git repo, and has (or can create) `features/`.
2. `workflow_docs_list` — list `docs/*/spec.md` in the working repo with `{ slug, promoted, target }` where `promoted` = a matching `features/*/<slug>/` exists in the docs repo.
3. `workflow_docs_promote { slug, confirmed, force? }` — copy `spec.md` + `plan.md` (if present) into `<docs-repo>/features/YYYY-MM-<slug>/`, generate the feature `README.md` (summary from spec Context/Goals, affected-repos section, status table), and add/update the row in `<docs-repo>/features/README.md` index.
...
1. User runs `workflow_docs_repo_link { path }` once → config written.
2. User runs `workflow_docs_list` → sees local specs and promotion status.
