...
- Make plan completion part of the execution contract: every subagent-driven or inline run ends with `workit_plan_complete` (or the CLI equivalent) once the ledger is complete and verification passes, with the skill contract saying so explicitly.
...
flowchart LR
  ...
  verify["workit_verify"]
  complete["workit_plan_complete"]
...
| Plan completion | The `active|paused -> completed` lifecycle transition (`workit_plan_complete` / `workit flow complete`) that runs the SDD ledger check and repository verification before storing `completed`. |
...
| Completion | Executing agents call `workit_plan_complete` after the final task once verification passes; the skill contract mandates it, and `workit_plan_complete`'s existing gates (complete ledger, green verification) are the only conditions. |
...
- CA-01: wk-implement, wk-handoff, and subagent-driven skill contracts explicitly require ending execution with `workit_plan_complete` (or CLI equivalent) after the final task, ledger completion, and green verification; no run may finish while the plan is `active`.
