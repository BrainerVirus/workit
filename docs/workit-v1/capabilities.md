...
| known_product_writes | enforced | enforced | enforced | enforced | — | — |
...
- **known_product_writes** (enforced): known write surfaces check the authoritative core writer before execution
...
- **known_product_writes** (enforced): Cursor preToolUse can synchronously deny recognized Write/Edit/Delete targets
...
- **known_product_writes** (enforced): PreToolUse denies covered Bash/apply_patch and local write targets only when the hooks.json bundle is installed; uncovered inputs stay agent_guided
...
- **product_write_interception** (enforced): Pi exposes a before-tool boundary for known built-in write tools.
