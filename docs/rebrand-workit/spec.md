...
- G5b: Code-review CI evaluated and REJECTED (user decision): the opencode GitHub review workflow with `opencode-go/deepseek-v4-flash` works (the GitHub App is installed, the model streams, comments post) but the review agent explores the whole repo and takes 12+ minutes on a large PR — far above the user's 3-minute budget (they expected coderabbit.ai-style speed). The workflow was removed; the per-package check jobs remain the quality gate. An external fast reviewer (e.g. CodeRabbit) is documented in README as the future option; Cursor Bugbot also remains optional.
...
- G8 (DONE): GitHub repo renamed `workflow-toolkit` → `workit` (BrainerVirus/workit, remote URL updated, secrets + branch protection + release migrated automatically).
...
- CA-07 (DONE): GitHub repo renamed to `workit` (BrainerVirus/workit); `git remote get-url origin` = git@github.com:BrainerVirus/workit.git; secrets (NPMJS, OPENCODE_API_KEY), branch protection (7 checks) and release v0.4.0 migrated automatically.
...
- CA-10: README documents the code-review decision — the opencode GitHub review workflow was evaluated and removed (too slow: 12+ min on large PRs vs the 3-min budget); the per-package CI checks are the required gate; CodeRabbit/Bugbot listed as optional fast reviewers.
